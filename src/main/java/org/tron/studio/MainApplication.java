...
public class MainApplication extends Application {
  ...
  {
    TabPane codeAreaTabPane = ShareData.currentContractTab.getTabPane();
    ...
    Runnable rn = ()-> {
      ...
      {
        vbox = (VBox) searchPopup.getPopupContent();

        if (vbox.getChildren().size() == 2)
        {
          vbox.getChildren().remove(1);
          searchPopup.setPopupContent(vbox);
          searchPopup.show(codeAreaTabPane, JFXPopup.PopupVPosition.TOP, JFXPopup.PopupHPosition.RIGHT);
          return;
        }
